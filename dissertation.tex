...
The GCM used is the NCEP Global Forecast System. 
...
\begin{enumerate}
    ...
    \item use GCM to predict the state of the atmosphere at the next timestep
\end{enumerate}
...
observations for the current timestep, to produce 56 estimates for the state of the atmosphere based on the
GCM and the observations. These estimates are then used as the starting point for the GCMs, which
predict what the state of the atmosphere will be in 6 hours time, and this is used as the background
state for the EKF, which assimilates the pressure observations at the new timestep. 
...
far more important if one were looking at tropical cyclones over the Pacific, when this is an active
period.


% Assimilates the observations and the output from the GCM using an EKF to provide an estimate as to the state of
% the atmosphere at each timestep
