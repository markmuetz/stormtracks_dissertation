...
\begin{center}
\textbf{Word count:} 10540 % 9293 + 1247 (captions) 26/08/14 14:19
\end{center}
...
also evident that the best tracks hurricane timesteps is substantially larger in the aircraft (and
early satellite) era.
...
the reverse being true in the post-1944 period.
...
\begin{figure}[hb!]
    ...
    lower. The pre-1944 period has a gradient of \SI{1.04}{} and a y-intercept of \SI{15.31}{}. The
    post-1944 period has a slop of \SI{0.83}{} and a y-intercept of \SI{-8.52}{}. }
...
on the four eras listed in Section TODOREF. This again shows that the number of estimated hurricane
timesteps in the pre-Panama Canal era is far higher than in the best tracks database, both due to
the large positive y-intercept and the gradient which is greater than unity. Reasons for this are
discussed in Section TODOREF. In the Panama Canal era, the gradient is slightly below unity, and the
...
era, the estimated hurricane timesteps were almost always lower than the best tracks hurricane
timesteps, with the effect becoming more pronounced as the hurricane timesteps per year increases.
Reasons as to why this might be are discussed in Section TODOREF. Finally, in the satellite era, the
gradient is very close to unity, and the y-intercept is significantly negative, indicating that
during this era the estimated hurricane timesteps is systematically lower than the best tracks
hurricane timesteps. This is to be expected given that the classification was trained on data from
the last two decades from this era.
...
the 21\ts{th} century in the number of hurricane timesteps, or a rise in the frequency of hurricanes
...
\ref{fig:20th_century_hurricane_timesteps}), and therefore a ``rise'' could be due to
underestimating how many hurricanes there were in the late 1960s and 1970s, rather than a genuine
signal. The same era in the best tracks number is not seen to show any significant rise, although
...
cyclones in the output in T106 resolution reanalysis output. In this study, a metric called
...
lower (and therefore worse) than the minimum values of sensitivity listed in Table
...
ensemble members here. Therefore the sample size and consequently the reliability of these numbers
should be subject to less variability in this study.
...
non-hurricanes.  However, there were many fields that I would have liked to include but could not,
due to constraints as to what is easily available to download from the 20CR portal for each ensemble
member. These include the temperatures at \SI{700}{}, \SI{500}{} and \SI{300}{hPa}, which are used
to calculate the warm core criterion and temperature anomalies \parencite{bengtsson1995hurricane,
bengtsson1996will, walsh1997objective, yoshimura2006influence}. I would also have liked to include
the cyclone phase space parameters as described in \textcite{evans2003objective, hart2003cyclone},
but these require the geopotential heights of \SI{900}{}, \SI{600}{} and \SI{300}{hPa}, and these
are not available for each ensemble member either, although the geopotential heights of \SI{1000}{},
\SI{500}{} and \SI{200}{hPa} are.  Whilst these fields are available, the only way to obtain them is
to download the entire state of the atmosphere as represented by the GCM, which would prohibitively
time consuming for this project, although these extra fields would have probably helped the
...
% study. Talk about supervised machine learning with respect to hurricane detection, and some of the
